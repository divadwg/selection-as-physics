...
**Status in one line:** condition 2 (sorting) is a verified theorem with both bridges physically derived and real-system placements; condition 1 (heredity) is demonstrated, with copying-as-memory-refresh confirmed and the dent-life law sketched; condition 3 (establishment) is restructured as an escape race with its proper mathematics (branching diffusion) identified and its solution owed; condition 4 (portability) has a mechanism chain and a first-pass experiment showing gated tokens sufficient and necessary. Pre-copying, everything here is *sorting* (Vrba-Gould), not selection, and the write-ups say so.
...
## 3. The theorem in one non-living example, and where life enters
...
## 5. Rules (the objects selection acts on)
...
## 6a. Lemma L1 (irreversible maintenance)
...
## 7. Entropy: where it enters and where it does not
